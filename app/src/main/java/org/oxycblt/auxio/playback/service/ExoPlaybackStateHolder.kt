                ...
                val customCodecAdapterFactory = object : MediaCodecAdapter.Factory {
                    ...
                    override fun createAdapter(configuration: MediaCodecAdapter.Configuration): MediaCodecAdapter {
                        if (configuration.format.sampleMimeType == MimeTypes.AUDIO_AAC) {
                            ...
                            val drcLevel = if (playbackSettings.replayGainMode != ReplayGainMode.OFF) 72 else -1
                            configuration.mediaFormat.setInteger(
                                MediaFormat.KEY_AAC_DRC_TARGET_REFERENCE_LEVEL, drcLevel)
                            if (Build.VERSION.SDK_INT >= 30) {
                                val albumMode = if (playbackSettings.replayGainMode == ReplayGainMode.ALBUM) 1 else 0
                                configuration.mediaFormat.setInteger(
