...
class SeparatorsDialog : ViewBindingMaterialDialogFragment<DialogSeparatorsBinding>() {
        ...
        (savedInstanceState?.getString(KEY_PENDING_SEPARATORS) ?: musicSettings.separators)
            .forEach {
                when (it) {
                    ...
                    Separators.IDEOGRAPHIC_COMMA ->
                        binding.separatorIdeographicComma.isChecked = true
                    Separators.FULLWIDTH_AMPERSAND ->
                        binding.separatorFullwidthAmpersand.isChecked = true
    ...
    private fun getCurrentSeparators(): String {
        ...
        if (binding.separatorFullwidthAmpersand.isChecked)
            separators += Separators.FULLWIDTH_AMPERSAND
        return separators
    }
